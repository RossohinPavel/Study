...
class LinkedGraph:
    ...
    def _next(self, current, link_prev, curr_path, curr_links):
        curr_path += [current]
        if link_prev:
            curr_links += [link_prev]

        if current == self._stop_v:
            return curr_path, curr_links
        ...
        for link in current.links:
            path = []
            links = []
            if link.v1 not in curr_path:
                path, links = self._next(link.v1, link, curr_path[:], curr_links[:])
            elif link.v2 not in curr_path:
                path, links = self._next(link.v2, link, curr_path[:], curr_links[:])
